...
**Questions: the database is the truth, the file is its export.** Anyone can edit a
question in the app, so the file drifts the moment someone does — and
`seed-imported-questions.ts` skips prompts that already exist (deliberately: it must
never clobber a human's edit), so re-running it realigns nothing. A backup that
silently stops matching what it backs up is not a backup. The process:

```bash
pnpm tsx scripts/export-questions.ts --check   # report drift, exit 1 if any
pnpm tsx scripts/export-questions.ts           # rewrite the file from the DB
```

Run the export before committing question changes. Output is deterministic — sorted
by category, difficulty, prompt, with fixed key order — so two runs against the same
database give byte-identical files and the diff only ever shows real changes. Only
`mcq` and `open` are exported: `geo` is generated by `seed-geo-full.ts`, and
`image`/`sort` can carry uploaded media that lives in `UPLOAD_DIR` and cannot travel
in JSON.

Both files are written one record per line on purpose (`scripts/format-questions.ts`
does this for the questions). Pretty-printing them made a 200-question addition a
