/**
 * Creates questions from scripts/data/economy-questions.fr.json — 353 French economy and
 * finance questions (fiscal system, labor market, monetary history, financial markets, major
 * French corporates, economic history, difficulty 1-5), hand authored for this project (not
 * sourced from an external, independently-vetted dataset the way imported-questions.fr.json is).
 *
 ...
 *
 * Run: pnpm tsx scripts/seed-economy-questions.ts [--author alex] [--dry-run]
 */
...
async function main() {
  ...
  const file = JSON.parse(
    readFileSync(new URL("./data/economy-questions.fr.json", import.meta.url), "utf-8"),
  ...
  console.log(
    JSON.stringify({
      event: "seed_economy_questions_complete",
      created,
      skipped,
